...
<div id="mepr-groups-form">
  ...
  <label for="<?php echo MeprGroup::$is_upgrade_path_str; ?>"><?php _e('This group is a product upgrade path', 'memberpress'); ?></label>
  <?php
    MeprAppHelper::info_tooltip( 'mepr-group-page-upgrade-path',
                                 __('Upgrade Path', 'memberpress'),
                                 __('Enabling this will mean that users can only be subscribed to one of these products at a time. If un-checked, users may be subscribed to each of these products simultaneously.', 'memberpress') );
  ?>
  
  <?php //require(MEPR_VIEWS_PATH.'/groups/style-option-dropdowns.php'); ?>
  
  <h4><strong><?php _e('Products:', 'memberpress'); ?></strong>
  <?php
    MeprAppHelper::info_tooltip('mepr-group-products-list',
                              __('Products', 'memberpress'),
                              __('Here you can add/remove products from this group pricing page.', 'memberpress'));
  ?>
  </h4>
  <ol id="sortable-products" class="mepr-sortable">
    <?php MeprGroupsHelper::get_existing_products_list($group); ?>
  </ol>
  <a href="" id="add-new-product" title="<?php _e('Add Product', 'memberpress'); ?>"><i class="mp-icon mp-icon-plus-circled mp-24"></i></a>
  <div id="hidden-line-item">
    <li class="product-item">
      <?php MeprGroupsHelper::get_products_dropdown(); ?>
      <span class="remove-span">
        <a href="" class="remove-product-item" title="Remove Product"><i class="mp-icon mp-icon-cancel-circled mp-16"></i></a>
      </span>
    </li>
  </div>
  ...
  <?php
    MeprAppHelper::info_tooltip('mepr-group-disable-pricing-page',
                                ...
                                __('This will disable the pricing page from being accessed on the front end of your site. It will return a 404 (not found) page if a user attempts to access it.', 'memberpress'));
  ?>
  <br/>
