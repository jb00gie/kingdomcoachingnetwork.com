...
<div id="mepr-account-nav">
  <span class="mepr-nav-item <?php MeprAccountHelper::active_nav('home'); ?>">
    <a href="<?php echo apply_filters('mepr-account-nav-home-link',$account_url.$delim.'action=home'); ?>" id="mepr-account-home"><?php _e('Home', 'memberpress'); ?></a>
  </span>
  <span class="mepr-nav-item <?php MeprAccountHelper::active_nav('subscriptions'); ?>">
    <a href="<?php echo apply_filters('mepr-account-nav-subscriptions-link',$account_url.$delim.'action=subscriptions'); ?>" id="mepr-account-subscriptions"><?php _e('Subscriptions', 'memberpress'); ?></a></span>
  <span class="mepr-nav-item <?php MeprAccountHelper::active_nav('payments'); ?>">
    <a href="<?php echo apply_filters('mepr_account-nav-payments-link',$account_url.$delim.'action=payments'); ?>" id="mepr-account-payments"><?php _e('Payments', 'memberpress'); ?></a>
  </span>
  <?php do_action('mepr_account_nav', $mepr_current_user); ?>
...
if(isset($expired_subs) and !empty($expired_subs)) {
  $account_url = get_permalink(); // $mepr_options->account_page_url();
