...
<li class="mepr-custom-field postbox">
  ...
  <input type="text" name="mepr-custom-fields[<?php echo $random_id; ?>][name]" value="<?php echo stripslashes($line->field_name); ?>" />
  ...
  <label><?php _e('Default Value:', 'memberpress'); ?></label>
  ...
  <?php
    if(empty($hide))
    {
      echo '<ul class="custom_options_list">';
      ...
      echo '<a href="" id="mepr-add-new-option" title="Add Option" data-value="'.$random_id.'"><i class="mp-icon mp-icon-plus-circled mp-16"></i></a>';
      echo '</ul>';
    }
  ?>
  </div>
